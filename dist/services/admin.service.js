 ...
 */
function adminMediaUrl(kind, id, token) {
 ...
 */
function imageRef(kind, id, value, token) {
    if (!value)
        return null;
    if (id && value.startsWith('data:'))
        return adminMediaUrl(kind, id, token);
    return value;
}
class AdminService {
    async getStats() {
        const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const [totalUsers, totalCouples, totalCommunities, _totalMatches, totalPrompts, pendingReports, activeToday, bannedCouples,] = await Promise.all([
            prisma_1.prisma.user.count(),
            prisma_1.prisma.couple.count(),
            prisma_1.prisma.community.count(),
            prisma_1.prisma.match.count({ where: { status: 'accepted' } }),
            prisma_1.prisma.prompt.count({ where: { isActive: true } }),
            prisma_1.prisma.report.count({ where: { status: 'pending' } }),
            // Real activity now: pinged within last 24h via lastActiveAt.
            prisma_1.prisma.user.count({ where: { lastActiveAt: { gte: dayAgo } } }),
            prisma_1.prisma.couple.count({ where: { bannedAt: { not: null } } }),
        ]);
        return {
            totalUsers,
            totalCouples,
    ...
    async getCouples(token) {
        ...
        return couples.map((c, idx) => {
            ...
            if (c.bannedAt)
                status = 'banned';
            else if (c.isProfileComplete && bothInactive)
                status = 'inactive';
    ...
    async getRawImage(kind, id) {
        if (kind === 'couple') {
            const c = await prisma_1.prisma.couple.findUnique({
                where: { coupleId: id },
                select: { primaryPhoto: true },
            });
            return c?.primaryPhoto ?? null;
        }
        const cm = await prisma_1.prisma.community.findUnique({
            where: { id },
    ...
    async getCityDistribution() {
        ...
        const [users, couples] = await Promise.all([
            prisma_1.prisma.user.findMany({ include: { coupleProfile: true } }),
            prisma_1.prisma.couple.findMany(),
        ]);
        users.forEach((u, idx) => {
    ...
    async deleteCouple(coupleId) {
        ...
        const userIds = [couple.partner1Id, couple.partner2Id].filter(Boolean);
    ...
    async getActivities() {
        const [notifs, users, communities] = await Promise.all([
            ...
            prisma_1.prisma.user.findMany({ orderBy: { createdAt: 'desc' }, take: 10 }),
            prisma_1.prisma.community.findMany({ orderBy: { createdAt: 'desc' }, take: 10 }),
        ]);
        const activities = [];
        notifs.forEach(n => {
            activities.push({
    ...
    async getPrompts() {
        const list = await prisma_1.prisma.prompt.findMany({
    ...
    async getReports() {
        const list = await prisma_1.prisma.report.findMany({
            orderBy: { createdAt: 'desc' },
            include: { reporter: true },
        });
        const reportsWithTargets = await Promise.all(list.map(async (r) => {
            let targetName = 'Unknown Target';
            const cp = await prisma_1.prisma.couple.findUnique({ where: { coupleId: r.targetId }, select: { profileName: true } });
            if (cp) {
                targetName = cp.profileName || 'Anonymous Couple';
            }
            else {
                const cm = await prisma_1.prisma.community.findUnique({ where: { id: r.targetId }, select: { name: true } });
                if (cm)
                    targetName = cm.name;
            }
            return {
    ...
    async getBlocks() {
        ...
        for (const c of couples) {
            for (const blockedId of c.blocked) {
                ...
                const cp = await prisma_1.prisma.couple.findFirst({
    ...
    async getChartData() {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const [u, c, comm] = await Promise.all([
                prisma_1.prisma.user.count({ where: { createdAt: { lte: endOfMonth } } }),
                prisma_1.prisma.couple.count({ where: { createdAt: { lte: endOfMonth } } }),
                prisma_1.prisma.community.count({ where: { createdAt: { lte: endOfMonth } } }),
            ]);
    ...
    async createCommunity(data) {
        ...
        if (data.hostCoupleId) {
            const host = await prisma_1.prisma.couple.findUnique({
                where: { coupleId: data.hostCoupleId },
                select: { coupleId: true },
            });
            hostExists = !!host;
        }
        return prisma_1.prisma.community.create({
            data: {
                name: data.name,
                description: data.description,
                city: data.city,
                tags: data.tags || [],
                coverImageUrl: (await (0, storage_1.materializeImageLoose)(data.coverImageUrl ?? data.coverImageBase64, data.hostCoupleId ?? undefined)) ?? undefined,
     ...
     */
    async processJoinRequestAsAdmin(communityId, requestId, decision) {
        const request = await prisma_1.prisma.communityJoinRequest.findUnique({
            where: { id: requestId },
        });
        if (!request || request.communityId !== communityId) {
            throw new Error('Join request not found');
        }
        await prisma_1.prisma.communityJoinRequest.delete({ where: { id: requestId } });
        if (decision === 'accept') {
     ...
     */
    async banCouple(coupleId, reason) {
        const couple = await prisma_1.prisma.couple.update({
            where: { coupleId },
            data: {
                bannedAt: new Date(),
        ...
        (0, authenticate_1.invalidateBanCache)(coupleId);
        // Also revoke any active refresh tokens so previously-issued sessions die.
        await prisma_1.prisma.user.updateMany({
            where: { coupleId },
            data: { refreshTokenHash: null },
        });
        logger_1.logger.warn(`[Admin] Banned couple ${coupleId} (reason: ${reason || 'n/a'})`);
        return couple;
    }
    /**
     * Unban a previously-banned couple. They can log in again immediately.
     */
    async unbanCouple(coupleId) {
        const couple = await prisma_1.prisma.couple.update({
            where: { coupleId },
            data: { bannedAt: null, banReason: null },
        });
        (0, authenticate_1.invalidateBanCache)(coupleId);
        logger_1.logger.info(`[Admin] Unbanned couple ${coupleId}`);
        return couple;
    }
    async addPrompt(text, category) {
        // Place new prompts at the end of their category's list
        const last = await prisma_1.prisma.prompt.findFirst({
            where: { category },
            orderBy: { sortOrder: 'desc' },
            select: { sortOrder: true },
        });
        const nextOrder = (last?.sortOrder ?? -1) + 1;
        return prisma_1.prisma.prompt.create({ data: { text, category, sortOrder: nextOrder } });
    }
    async togglePrompt(id) {
        const p = await prisma_1.prisma.prompt.findUnique({ where: { id } });
        if (!p)
            throw new Error('Prompt not found');
        return prisma_1.prisma.prompt.update({ where: { id }, data: { isActive: !p.isActive } });
    }
    async editPrompt(id, text) {
        const p = await prisma_1.prisma.prompt.findUnique({ where: { id } });
        if (!p)
            throw new Error('Prompt not found');
        return prisma_1.prisma.prompt.update({ where: { id }, data: { text } });
    }
    async reorderPrompts(ids) {
        // ids is the desired order; update sortOrder for each
        await prisma_1.prisma.$transaction(ids.map((id, index) => prisma_1.prisma.prompt.update({ where: { id }, data: { sortOrder: index } })));
    }
    async deletePrompt(id) {
        return prisma_1.prisma.prompt.delete({ where: { id } });
    }
    async sendNotification(title, message, recipientIds) {
        let validCoupleIds;
        if (recipientIds && recipientIds.length > 0) {
            // Validate — only keep IDs that actually exist in the couples table
            const existing = await prisma_1.prisma.couple.findMany({
            ...
            const allCouples = await prisma_1.prisma.couple.findMany({
                where: { coupleId: { not: '' } },
                select: { coupleId: true },
            });
        ...
        const result = await prisma_1.prisma.notification.createMany({ data, skipDuplicates: true });
        // Real-time fan-out: in-app socket + OS push (FCM) per recipient.
        for (const coupleId of validCoupleIds) {
            (0, realtime_1.emitRealtimeNotification)(coupleId, {
                type: 'admin',
                title,
                message,
            });
