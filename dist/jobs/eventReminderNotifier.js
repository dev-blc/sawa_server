...
exports.startEventReminderNotifier = void 0;
...
async function runCheck() {
    ...
    try {
        ...
        const events = await prisma_1.prisma.plannedDate.findMany({
        ...
        for (const ev of events) {
            try {
                ...
                const { title, body } = (0, notif_1.renderNotif)('en', 'us.date.reminder', params); // client re-localizes
                await prisma_1.prisma.notification.create({
                    data: {
                        ...
                        data: {
                            ...
                            ...(0, notif_1.i18nData)('us.date.reminder', params),
                        },
                        read: false,
                    },
                });
                ...
                const io = global.io;
                if (io)
                    io.to(`couple:${coupleId}`).emit('notification:new', { type: 'us_date_reminder' });
                ...
                sent += 1;
                logger_1.logger.info(`[EventReminder] reminded couple ${coupleId} about "${activity}" on ${ev.rawDate}`);
            }
            catch (err) {
                logger_1.logger.warn(`[EventReminder] couple ${ev.coupleId} failed: ${err.message}`);
            }
        }
        if (sent)
            logger_1.logger.info(`[EventReminder] sent ${sent} date reminder(s) for ${tomorrow}`);
    }
    catch (err) {
        logger_1.logger.warn(`[EventReminder] run failed: ${err.message}`);
    }
}
/** Start the notifier — check shortly after boot, then every 3 hours. */
const startEventReminderNotifier = () => {
    setTimeout(() => runCheck().catch(() => { }), 20000); // after sockets/db settle
    setInterval(() => runCheck().catch(() => { }), 3 * 60 * 60 * 1000);
    logger_1.logger.info('📅 Event reminder notifier scheduled (every 3h, 08–21 IST)');
};
exports.startEventReminderNotifier = startEventReminderNotifier;
//# sourceMappingURL=eventReminderNotifier.js.map